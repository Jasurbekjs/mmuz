	...
	if(!empty($logistic_id))
	{
		...
		if(!empty($CARRIER_ID) && !empty($DRIVER_ID) && !empty($TRANSPORT_ID))
		{
			...
			$Insert_shipping=mysqli_query($connection,
				"UPDATE `shipping` SET 	`logistics_id`='$num_logistic_id',
										`carrier_id`='$num_CARRIER_ID',
										`driver_id`='$num_DRIVER_ID',
										`transport_id`='$num_TRANSPORT_ID',
										`rate`='$double_rate',
										`rate_currency`='$string_rate_currency' 
										WHERE `id`='$num_transport_id'");	
