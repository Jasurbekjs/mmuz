<?php
	...
	if(!empty($carrier_id))
	{
	    $uploaddir = '../files/orders/';
	    $name='Order_'.$id.'_'.$carrier_id.'_'.translit(basename($_FILES['file']['name']));
	...
	if($move && !empty($id))
	{
		...
		if($Orders==NULL)
		{
			$Insert_order_file=mysqli_query($connection,"
			INSERT INTO `orders`(`id`,`logistics_id`, `carrier_id`, `contract_id`, `order_number`, `order_date`, `path`) 
			VALUES (NULL, '$id', '$carrier_id', '', '', '2000-01-01', '$name')");
		}
		elseif ($Orders!=NULL)
		{
			$Update_order_file=mysqli_query($connection,"
			UPDATE `orders` SET 
			`path`='$name' WHERE `logistics_id`='$id' AND `carrier_id`='$carrier_id'");
		}
